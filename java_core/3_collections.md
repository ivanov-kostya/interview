...
### Когда массив считается заполненным?
...
1. Расскажите про иерархию коллекций в Java. Основные интерфейсы и их реализации.

В Java коллекции представляют собой структуру данных, которая позволяет хранить и обрабатывать группы объектов. Иерархия коллекций в Java организована в виде набора интерфейсов и классов, которые обеспечивают различные способы хранения, обработки и управления данными.

### Основные интерфейсы коллекций в Java

Иерархия коллекций в Java основывается на двух основных интерфейсах: `Collection` и `Map`.

1. **Collection**:
  - Это корневой интерфейс для всех коллекций в Java. Он определяет базовые операции, такие как добавление, удаление и итерация по элементам.
  - Подинтерфейсы:
    - **List**: Упорядоченная коллекция, которая может содержать дубликаты. Элементы имеют индекс.
      - **Основные реализации**:
        - `ArrayList`: Реализация на основе массива. Позволяет быстрый доступ к элементам по индексу.
        - `LinkedList`: Реализация на основе двусвязного списка. Обеспечивает более быструю вставку и удаление элементов.
    - **Set**: Коллекция, которая не допускает дубликатов. Элементы не имеют индекса и не имеют определенного порядка.
      - **Основные реализации**:
        - `HashSet`: Использует хеш-таблицу для хранения элементов. Обеспечивает быстрые операции добавления, удаления и поиска.
        - `LinkedHashSet`: Как и `HashSet`, но сохраняет порядок вставки элементов.
        - `TreeSet`: Реализация на основе красно-черного дерева, которая хранит элементы в отсортированном порядке.
    - **Queue**: Коллекция, которая используется для хранения элементов, ожидающих обработки. Поддерживает порядок FIFO (first-in, first-out).
      - **Основные реализации**:
        - `PriorityQueue`: Реализация, которая обрабатывает элементы на основе их приоритета.
        - `LinkedList`: Также может использоваться как очередь.

2. **Map**:
  - Интерфейс, который представляет собой коллекцию пар "ключ-значение". Ключи уникальны, а значения могут дублироваться.
  - **Основные реализации**:
    - `HashMap`: Использует хеш-таблицу для хранения пар ключ-значение. Обеспечивает быстрый доступ по ключу.
    - `LinkedHashMap`: Как и `HashMap`, но сохраняет порядок вставки пар.
    - `TreeMap`: Реализация на основе красно-черного дерева, которая хранит пары в отсортированном порядке по ключам.
    - `Hashtable`: Устаревшая реализация, которая синхронизирована и не допускает `null` ключей или значений.

### Иерархия коллекций в Java

Ниже представлена общая структура иерархии коллекций в Java:
