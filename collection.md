...
| Интерфейс | Реализации | Упорядоченность | Дубликаты | Ключ-значение | Потокобезопасность | Производительность (в среднем) |
|-----------|------------|-----------------|-----------|----------------|---------------------|--------------------------------|
| List      | ArrayList  | Да              | Да        | Нет            | Нет                 | Доступ: O(1), Вставка/Удаление: O(n) |
|           | LinkedList | Да              | Да        | Нет            | Нет                 | Доступ: O(n), Вставка/Удаление: O(1) |
|           | Vector     | Да              | Да        | Нет            | Да                  | Доступ: O(1), Вставка/Удаление: O(n) |
| Set       | HashSet    | Нет             | Нет       | Нет            | Нет                 | O(1) для основных операций |
|           | LinkedHashSet | Да (порядок вставки) | Нет | Нет         | Нет                 | O(1) для основных операций |
|           | TreeSet    | Да (отсортировано) | Нет    | Нет            | Нет                 | O(log n) для основных операций |
| Queue     | PriorityQueue | Да (по приоритету) | Да  | Нет            | Нет                 | O(log n) для вставки, O(1) для просмотра |
|           | LinkedList | Да              | Да        | Нет            | Нет                 | O(1) для основных операций |
| Deque     | ArrayDeque | Да              | Да        | Нет            | Нет                 | O(1) для основных операций |
|           | LinkedList | Да              | Да        | Нет            | Нет                 | O(1) для основных операций |
| Map       | HashMap    | Нет             | Только значения | Да       | Нет                 | O(1) для основных операций |
|           | LinkedHashMap | Да (порядок вставки) | Только значения | Да | Нет              | O(1) для основных операций |
|           | TreeMap    | Да (отсортировано по ключу) | Только значения | Да | Нет          | O(log n) для основных операций |
|           | Hashtable  | Нет             | Только значения | Да       | Да                  | O(1) для основных операций |
